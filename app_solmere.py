...
def load_data():
    df = pd.read_csv(
        "project-2-behavioral-churn/solmere_churn_clean.csv",
        dtype={"HasNegativeTicketPreRenewal": "bool", "Cancelled": "bool"},
    )
    return df
...
region_filter = st.sidebar.selectbox("Select Geographical Region", ["All Regions"] + sorted(df["Region"].dropna().unique()))
plan_filter = st.sidebar.selectbox("Select Subscription Tier", ["All Plans"] + sorted(df["Plan"].dropna().unique()))
...
churn_count = int(filtered_df["Cancelled"].sum())
